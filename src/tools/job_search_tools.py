...
if __name__ == "__main__":
    """
    Test the JSearch tool
    
    Run: python -m src.tools.job_search_tool
    """
    print("🔍 Testing JSearch API Tool...\n")
    print("=" * 60)
    
    # Check if API key is configured
    if not settings.rapidapi_key or settings.rapidapi_key == "your_api_key_here":
        print("❌ Error: RAPIDAPI_KEY not configured in .env file")
        print("\nPlease add your RapidAPI key to .env:")
        print("RAPIDAPI_KEY=your_actual_key_here")
        exit(1)
    
    try:
        # Test 1: Basic search
        print("\n🧪 Test 1: Searching for 'Python Developer' jobs...\n")
        tool = JobSearchTool()
        results = tool._run(query="Python Developer", num_results=3)
        ...
        # Test 2: Convert to JobPosting objects
        print("\n🧪 Test 2: Converting to JobPosting objects...\n")
        job_postings = search_jobs_for_candidate(
            job_title="Data Scientist",
            skills=["Python", "Machine Learning"],
            num_results=2
        )
        
        print(f"✅ Created {len(job_postings)} JobPosting objects\n")
        
        for job in job_postings:
            print(f"  - {job.title} at {job.company}")
        
        print("\n" + "=" * 60)
        print("🎉 JSearch tool is working correctly!")
        print(f"📊 API calls remaining: Check your RapidAPI dashboard")
