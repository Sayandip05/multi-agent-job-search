...
if __name__ == "__main__":
    """
    Test the skill matching agent
    
    Run: python -m src.agents.skill_matcher
    """
    ...
    # Create a sample candidate (simplified version)
    candidate = CandidateProfile(
        name="John Doe",
    ...
    job = JobPosting(
        ...
        required_skills=["Python (3+ years)", "Django", "PostgreSQL"],
    ...
    try:
        # Perform matching
        print("\n🔍 Analyzing candidate vs job...\n")
        match_result = match_candidate_to_job(candidate, job)
        
        # Display results
        print("✅ Match Analysis Complete!\n")
        print(f"Overall Fit Score: {match_result.overall_fit_score}/100")
        print(f"Skill Match Score: {match_result.skill_match_score}/60")
        print(f"Experience Match Score: {match_result.experience_match_score}/30")
        print(f"\nRecommendation: {match_result.recommendation}")
        
        print(f"\n💪 Strengths ({len(match_result.strengths)}):")
        for strength in match_result.strengths:
            print(f"  - {strength}")
        
        print(f"\n⚠️  Gaps ({len(match_result.gaps)}):")
        ...
        print("\n" + "=" * 60)
        print("🎉 Skill Matching Agent is working correctly!")
